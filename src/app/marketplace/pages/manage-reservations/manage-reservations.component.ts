...
export class ManageReservationsComponent implements OnInit {
  ...
  constructor(private reservationService: ReservationVisiteService,
  ...
  loadReservations() {
    this.reservationService.getByOwner(this.currentUserId).subscribe({
      next: (data: any) => {
        ...
        this.reservations = data.filter((r: any) =>
...
getStatusClass(status: string): string {
  switch (status) {
    ...
    case 'ANNULEE': return 'status-refused';
    case 'TERMINEE': return 'status-done';
    default: return '';
  }
}
...
getStatusLabel(status: string): string {
  switch (status) {
    ...
    case 'CONFIRMEE': return 'Accepted';
    case 'ANNULEE': return 'Refused';
